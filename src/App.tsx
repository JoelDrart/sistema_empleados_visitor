...
const App: React.FC = () => {
    ...
    async function getEmployees() {
        try {
            const response = await axios.get(API_ROUTES.GET_EMPLOYEES);
    ...
    const handleSaveEmployee = async (updatedEmployee: Employee) => {
        try {
            await axios.put(`${API_ROUTES.UPDATE_EMPLOYEE}/${updatedEmployee.id}`, updatedEmployee);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {employees.map((employee, index) => (
                    ...
                    >
                        <EmployeeCard
                            employee={employee}
                            salary={employee.accept(salaryCalculator)}
                        />
                    </div>
                ))}
                <AddEmployeeCard
