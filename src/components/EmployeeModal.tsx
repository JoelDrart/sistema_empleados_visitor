...
const EmployeeModal: React.FC<EmployeeModalProps> = ({ employee, isOpen, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50  flex justify-center items-center">
      <div className="bg-white p-6 rounded shadow-lg w-96">
        ...
        <span>Nombre: </span>
        <input className="w-full border p-2 mb-2" type="text" value={name} onChange={(e) => setName(e.target.value)} />
        
        {employee instanceof SalariedEmployee ? (
            <>
            
            <span>Meses trabajados: </span>
          <input className="w-full border p-2 mb-2" type="number" value={monthsWorked} onChange={(e) => setMonthsWorked(Number(e.target.value))} />
            </>
        ) : (
          <>
          <span>Pago/hora: </span>
            <input className="w-full border p-2 mb-2" type="number" value={hourlyRate} onChange={(e) => setHourlyRate(Number(e.target.value))} />
            <span>Horas trabajadas: </span>
            <input className="w-full border p-2 mb-2" type="number" value={hoursWorked} onChange={(e) => setHoursWorked(Number(e.target.value))} />
          </>
        )}
